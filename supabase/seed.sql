...
begin
  ...
  select id into v_company_2 from companies where owner_id = p_owner_id and name = 'Studio Forma' limit 1;
  ...
  select id into v_contact_1 from contacts where owner_id = p_owner_id and email = 'm.rossi@edilnova.it' limit 1;
  ...
  insert into opportunities (
    ...
    (p_owner_id, v_company_2, v_contact_2, 'Restyling showroom Roma', 'Opportunita retail da finalizzare', 'proposal', 18000, 55, 'Website', current_date + 14, 'Inviare revisione offerta', now() + interval '1 day'),
  ...
  select id into v_opportunity_1 from opportunities where owner_id = p_owner_id and title = 'Fornitura serramenti Milano' limit 1;
  ...
  insert into activities (
    owner_id, company_id, contact_id, opportunity_id, kind, subject, content, happened_at, created_by
  ) values
    (p_owner_id, v_company_1, v_contact_1, v_opportunity_1, 'call', 'Call iniziale', 'Cliente interessato, richiede tempi di consegna certi', now() - interval '6 day', p_owner_id),
    (p_owner_id, v_company_1, v_contact_1, v_opportunity_1, 'meeting', 'Incontro tecnico', 'Confermate specifiche principali e range budget', now() - interval '3 day', p_owner_id),
    (p_owner_id, v_company_2, v_contact_2, v_opportunity_2, 'email', 'Invio proposta', 'Inviata proposta economica aggiornata', now() - interval '2 day', p_owner_id),
    (p_owner_id, v_company_3, v_contact_3, v_opportunity_3, 'whatsapp', 'Primo contatto', 'Scambiato messaggio, disponibile a sentirci la prossima settimana', now() - interval '1 day', p_owner_id);

  insert into followups (
    owner_id, company_id, contact_id, opportunity_id, title, description,
    due_at, status, priority, reminder_at, created_by
  ) values
    (p_owner_id, v_company_1, v_contact_1, v_opportunity_1, 'Richiamo finale trattativa', 'Verificare feedback economico e tempi', now() + interval '1 day', 'pending', 'high', now() + interval '20 hour', p_owner_id),
    (p_owner_id, v_company_2, v_contact_2, v_opportunity_2, 'Invio revisione proposta', 'Mandare variante con tempi aggiornati', now() + interval '6 hour', 'in_progress', 'urgent', now() + interval '5 hour', p_owner_id),
    (p_owner_id, v_company_3, v_contact_3, v_opportunity_3, 'Fissare call di qualificazione', 'Portare il lead a meeting conoscitivo', now() - interval '4 hour', 'pending', 'medium', now() - interval '6 hour', p_owner_id);
  ...
  select id into v_tag_hot from tags where owner_id = p_owner_id and name = 'Hot' limit 1;

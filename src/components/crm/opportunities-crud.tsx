...
export function OpportunitiesCrud({ opportunities, companies, contacts }: { opportunities: any[]; companies: any[]; contacts: any[] }) {
  ...
  const availableContacts = useMemo(() => {
    ...
    return contacts.filter((contact) => contact.company_id === selectedCompanyId)
          ...
          <div className="sheet-card">
            ...
            <form action={handleCreate} className="sheet-form">
              <div className="form-grid two-col">
                ...
                <label className="field-stack"><span>Contatto</span><select className="field-control" name="primary_contact_id" defaultValue="" disabled={!selectedCompanyId}><option value="">{selectedCompanyId ? 'Nessuno' : 'Seleziona prima azienda'}</option>{availableContacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.first_name} {contact.last_name}</option>)}</select></label>
                ...
                <label className="field-stack"><span>Probabilità %</span><input className="field-control" name="probability" type="number" min="0" max="100" step="1" placeholder="0-100" /></label>

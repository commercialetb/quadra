...
export function CompanyAnalysisCard({ data }: { data: CompanyAnalysisData | null }) {
  ...
  return (
    <section className="panel-card company-analysis-card">
      ...
      <div className="company-analysis-metrics">
        <div className="company-analysis-metric"><span>Pipeline</span><strong>{formatCurrency(data.companyRow.pipelineValue)}</strong></div>
        <div className="company-analysis-metric"><span>Ordini importati</span><strong>{data.companyRow.importedOrders}</strong></div>
        <div className="company-analysis-metric"><span>Valore ordini</span><strong>{formatCurrency(data.companyRow.importedValue)}</strong></div>
        <div className="company-analysis-metric"><span>Follow-up attivi</span><strong>{data.companyRow.pendingFollowups}</strong></div>
      </div>

      <div className="company-analysis-score-grid">
        <div className="company-analysis-score-card">
          <span>Score priorità</span>
          <strong>{data.companyRow.priorityScore}/100</strong>
          <small>{data.companyRow.priorityBand === 'alta' ? 'Alta priorità' : data.companyRow.priorityBand === 'media' ? 'Priorità media' : 'Presidio base'}</small>
        </div>
        <div className="company-analysis-score-card">
          <span>Score potenziale</span>
          <strong>{data.companyRow.potentialScore}/100</strong>
          <small>Valore combinato tra pipeline, ordini e qualità relazione.</small>
        </div>
        <div className="company-analysis-score-card">
          <span>Score rischio</span>
          <strong>{data.companyRow.riskScore}/100</strong>
          <small>Pesa ritardi, assenza agenda e opportunità ferme.</small>
        </div>
      </div>

      <div className="company-analysis-pressure-strip">
      ...
      <div className="company-analysis-grid">
        <div className="company-analysis-block company-analysis-priority-block">
          ...
          {topAction ? (
            <div className="simple-list compact-list">
              <div className="simple-row static company-analysis-suggestion-row">
                <div>
        ...
        <div className="company-analysis-block">
          ...
          <div className="simple-list compact-list">
            <div className="simple-row static"><div><strong>Ultimo ordine</strong><span>{formatDate(data.companyRow.lastOrderDate)}</span></div></div>
            <div className="simple-row static"><div><strong>Outstanding</strong><span>{formatCurrency(data.orderKpis.outstandingValue)}</span></div></div>
            <div className="simple-row static"><div><strong>Ticket medio</strong><span>{formatCurrency(data.orderKpis.averageOrderValue)}</span></div></div>
            <div className="simple-row static"><div><strong>Ultimo mese letto</strong><span>{latestMonth ? `${latestMonth.label} · ${formatCurrency(latestMonth.value)}` : 'Nessun mese disponibile'}</span></div></div>
          </div>
        </div>
      </div>
      ...
      <div className="company-analysis-grid">
        <div className="company-analysis-block">
          ...
          <div className="simple-list compact-list">
            {data.suggestions.length > 0 ? data.suggestions.map((item) => (
        ...
        <div className="company-analysis-block">
          ...
          <div className="simple-list compact-list">
            {data.openOpportunities.length > 0 ? data.openOpportunities.map((item) => (
            ...
            {data.activeFollowups.length > 0 ? data.activeFollowups.map((item) => (

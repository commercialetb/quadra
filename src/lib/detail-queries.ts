...
export async function getProjectDetail(id: string) {
  ...
  const [{ data: project }, { data: notes }] = await Promise.all([
    supabase.from('projects').select('*, company:companies(id, name), opportunity:opportunities(id, title, stage)').eq('id', id).single(),
    supabase.from('notes').select('id, title, body, created_at').eq('entity_type', 'project').eq('entity_id', id).order('created_at', { ascending: false }).limit(10),
  ]);
...
export async function getTimelineForEntity(args: {
  ...
  if (args.projectId) {
    activityQuery = activityQuery.eq('project_id', args.projectId);
    followupQuery = followupQuery.eq('project_id', args.projectId);
  }
